...
public class LauncherModel extends BroadcastReceiver {
    ...
    FolderInfo getFolderById(Context context, HashMap<Long, FolderInfo> folderList, long id) {
        final ContentResolver cr = context.getContentResolver();
        Cursor c = cr.query(Favorites.CONTENT_URI, null,
...
//		        				+ appInfo.packageName + "/" + infoName.substring(infoName.indexOf(appInfo.packageName)+appInfo.packageName.length()) + ";end";
//      		
//      	} else {
//	        	intentInfo = "#Intent;action=android.intent.action.MAIN;category=android.intent.category.LAUNCHER;launchFlags=0x10200000;component=" 
...
//      	}
          ...
          //values2.put(Favorites.ITEM_TYPE, Favorites.ITEM_TYPE_APPLICATION);
	        ...
	        //Parcelable icon = Intent.ShortcutIconResource.fromContext(context.getApplicationContext(), info.getIconResource());  
	        //shortcutintent.putExtra(Intent.EXTRA_SHORTCUT_ICON_RESOURCE, bmp2); 
	        shortcutintent.putExtra(Intent.EXTRA_SHORTCUT_ICON, bmp);

	        //shortcutintent.putExtra(Intent.EXTRA_SHORTCUT_ICON_RESOURCE, ddk);
	        Intent intent2 = new Intent();  
	        intent2.setComponent(new ComponentName(appInfo.packageName, infoName)); 
	...
	public void onReceive(Context context, Intent intent) {
        ...
        if (mBeforeFirstLoad) {
        	...
        	if(Intent.ACTION_EXTERNAL_APPLICATIONS_AVAILABLE.equals(action)){
                Log.e(TAG, "onReceive, Before First Load, ACTION_EXTERNAL_APPLICATIONS_AVAILABLE");
        	}else{
        ...
        synchronized (mAllAppsListLock) {
            ...
            if (Intent.ACTION_PACKAGE_CHANGED.equals(action)
                ...
                if (Intent.ACTION_PACKAGE_CHANGED.equals(action)) {
					...
					if (packageName != null
							&& packageName.startsWith(ThemeUtils.THEME_PACKAGE_TOKEN)) {
                ...
                if (modified != null) {
                    ...
                    //createShortcutEx(context, intent); //yfzhao
                    //Toast.makeText(context, context.getString(R.string.app_updated),
                ...
                if (Intent.ACTION_EXTERNAL_APPLICATIONS_AVAILABLE.equals(action)) {
                     ...
                     if (DEBUG_LOADERS) Log.d(TAG, "ACTION_EXTERNAL_APPLICATIONS_AVAILABLE, size="+packages.length);
        ...
        private class LoaderThread extends Thread {
            ...
            private void loadWorkspace() {
                ...
                try {
                    ...
                    final int containerIndex = c.getColumnIndexOrThrow(Favorites.CONTAINER);
                    final int orderIdIndex = c.getColumnIndexOrThrow(BaseLauncherColumns.ORDERID);
                    final int itemTypeIndex = c.getColumnIndexOrThrow(BaseLauncherColumns.ITEM_TYPE);
                        ...
                        try {
                            ...
                            case BaseLauncherColumns.ITEM_TYPE_SHORTCUT:
                                ...
                                if (info != null) {
                                    ...
                                    contentResolver.delete(Favorites.getContentUri(id, false),
                                    		null, null);
                                }
                                break;
                            ...
                            case Favorites.ITEM_TYPE_WIDGET_CLEAN_MEMORY:
                            	CustomAppWidgetInfo customAppWidgetInfo = CustomAppWidgetInfo.getWidgetInfoByType(itemType);
                            	customAppWidgetInfo.id = c.getLong(idIndex);
                            	customAppWidgetInfo.screen = c.getInt(screenIndex);
                            	customAppWidgetInfo.cellX = c.getInt(cellXIndex);
                            	customAppWidgetInfo.cellY = c.getInt(cellYIndex);
                                ...
                                // check & update map of what's occupied
                                if (!checkItemPlacement(occupied, customAppWidgetInfo)) {
                                    break;
                                }
                ...
                if (itemsToRemove.size() > 0) {
                    ContentProviderClient client =
                    	contentResolver.acquireContentProviderClient(Favorites.CONTENT_URI);
                    // Remove dead items
            ...
            private void bindWorkspace() {
                ...
                for (int i = 0; i < N; i += ITEMS_CHUNK) {
                    ...
                    mHandler.post(new Runnable() {
						...
						public void run() {
                            if (DEBUG_LOADERS) {
                                Log.d(TAG, "bindWorkspace, bindItems, mStopped="+mStopped+", this="+this);
                            }
                            Callbacks callbacks = tryGetCallbacks(oldCallbacks);
                            if (callbacks != null) {
            ...
            private void updateSlideData(Context context) {
            	...
            	Log.e(TAG, "updateSlideData, count = " + count+", addCount="+addCount);
            ...
            int addPackageToSlideData(Context context, ContentResolver cr, int count) {
            	final Intent mainIntent = new Intent(Intent.ACTION_MAIN, null);
            	int lastPos = 0;
            	int add = 0;
                mainIntent.addCategory(Intent.CATEGORY_LAUNCHER);
                ...
                if (apps.size() > count) {
                	...
                	for (int i = 0; i < apps.size(); i++) {
		        		...
		        		// Do not add custom theme package to all application
		        		if (info.activityInfo.packageName.startsWith(ThemeUtils.THEME_PACKAGE_TOKEN) ||
		        				info.activityInfo.packageName.equals(selfPkgName)) {
			            	continue;
			            }
                        ...
                        if (c != null && !c.moveToNext()) {
            	        	...
            	        	Log.w(TAG, "addPackageToSlideData app=" + info.activityInfo.applicationInfo.packageName+", position="+(lastPos));
            	        	lastPos++;
            	        	add++;
                        }
                        c.close();
            ...
            int removePackageFromSlideData(Context context, ContentResolver cr) {
            	Cursor c = cr.query(Applications.CONTENT_URI_NO_NOTIFICATION,
        			    ...
        			    BaseLauncherColumns.ITEM_TYPE + " in " +
        			    		"(" + Applications.APPS_TYPE_APP + "," + Applications.APPS_TYPE_FOLDERAPP + ")",
        			    null,
        				null);
            	...
            	if (c != null) {
            		while (c.moveToNext()) {
            			String packageName = c.getString(c.getColumnIndex(Applications.PACKAGENAME));
            			if (!findPackage(packageName, packageManager)) {
            				ApplicationInfoEx applicationInfoEx = new ApplicationInfoEx();
                			applicationInfoEx.id = c.getInt(c.getColumnIndex(BaseColumns._ID));
                			applicationInfoEx.itemType = c.getInt(c.getColumnIndex(BaseLauncherColumns.ITEM_TYPE));
                			applicationInfoEx.packageName = packageName;
                			applicationInfoEx.position = c.getInt(c.getColumnIndex(Applications.POSITION));
            				mRemoveAppsList.add(applicationInfoEx);	
//            				Log.w(TAG, "removePackageFromSlideData app=" + packageName+", position="+(applicationInfoEx.position));
            			}
            		}
            		c.close();
            	}
            	...
            	for (int i = 0; i < mRemoveAppsList.size(); i++) {
            		...
            		c = cr.query(Applications.CONTENT_URI_NO_NOTIFICATION,
        		            ...
        		            Applications.POSITION},
            				Applications.PACKAGENAME + "=?",
            				new String[] { slideInfo.packageName },
            				null);
            		if(c != null && c.moveToNext()){
                		slideInfo.position = c.getInt(c.getColumnIndex(Applications.POSITION));
                		c.close();
            		}
            		...
            		if (slideInfo.itemType == Applications.APPS_TYPE_APP) {
            			cr.update(updateUri, null, null,
            					new String[] { String.valueOf(slideInfo.position) });
            		}

            		Log.w(TAG, "removePackageFromSlideData app=" + slideInfo.packageName+", position="+(slideInfo.position));
            ...
            boolean findPackage(String packageName, PackageManager packageManager){
    ...
    public ShortcutInfo getShortcutInfo(PackageManager manager, Intent intent, Context context,
        ...
        if (resolveInfo != null) {
        	...
        	Log.w(TAG, "getShortcutInfo error, resolveInfo null! intent="+intent);
//        	try {
...
//        	}
    ...
    Bitmap getIconFromCursor(Cursor c, int iconIndex) {
        if (DEBUG_LOADERS) {
            Log.d(TAG, "getIconFromCursor app="
                    + c.getString(c.getColumnIndexOrThrow(BaseLauncherColumns.TITLE)));
        }
    ...
    ShortcutInfo addShortcutInDock(Context context, Intent data,
        ...
        addItemToDatabase(context, info, Favorites.CONTAINER_DOCKBAR,
        		dockItemInfo.screen, dockItemInfo.cellX, dockItemInfo.cellY, notify);
