...
import com.vollo.launcher.theme.ThemeManager;
import com.vollo.launcher.widgets.LockScreenUtil;
 ...
 */
public class Workspace extends ViewGroup implements DropTarget, DragSource, DragScroller, LauncherMonitor.InfoCallback {

    private static final String TAG = "Launcher.Workspace";
    private static final int INVALID_SCREEN = -1;
    
    public static final String SCREEN_STATE = "com.vollo.launcher.screenstate";
    public static final String SCREEN_GETCONFIG = "com.vollo.launcher.getscreenconfig";
...
//        a.recycle();
    ...
    private void initWorkspace() {
		...
		mWidthStartPadding = (int) r.getDimension(R.dimen.workspace_cell_left_padding);
		mWidthEndPadding = (int) r.getDimension(R.dimen.workspace_cell_right_padding);
		mHeightStartPadding = (int) r.getDimension(R.dimen.workspace_cell_top_padding);
		mHeightEndPadding = (int) r.getDimension(R.dimen.workspace_cell_bottom_padding);
    ...
    public void setDrawCueNumberState(boolean draw, int type){
    	if(draw){
    		if(mCueBitmap == null){
    	    	BitmapDrawable  drawable = (BitmapDrawable) getResources().getDrawable(R.drawable.ic_cue_bg);
...
//        		Bitmap bmp = Utilities.createCompoundBitmapEx(fi.getText().toString(), icon);
//        		fi.setCompoundDrawablesWithIntrinsicBounds(null,  new FastBitmapDrawable(bmp), null, null);
//        	}
...
//        	}
    		...
    		Bitmap bmp = Utilities.createCompoundBitmapEx(customAppWidget.getText().toString(), icon);
    		
    		customAppWidget.setCompoundDrawablesWithIntrinsicBounds(null,  new FastBitmapDrawable(bmp), null, null);
        	...
        	Drawable d1 = d[1];
        	//if ((d1.getIntrinsicWidth() == 96)&&(d1.getIntrinsicHeight() == 96)) {
        		
        	//} else {
        		Bitmap icon = Utilities.drawable2bmp(d1);
        		Bitmap bmp = Utilities.createCompoundBitmapEx(btv.getText().toString(), icon);
        		//Bitmap bmp = Utilities.createBitmap4Launcher(icon);
        		btv.setCompoundDrawablesWithIntrinsicBounds(null,  new FastBitmapDrawable(bmp), null, null);
        	//}
        	//Bitmap icon = null;
        	//Bitmap icon = Utilities.drawable2bmp(d[1]);
        	//btv.setCompoundDrawablesWithIntrinsicBounds(null, new FastBitmapDrawable(icon), null, null);
    ...
    private void updateWallpaperOffset(int scrollRange) {
        ...
        if (token != null) {        	
            ...
            if (mTouchDirection==TOUCH_STATE_SCROLLING_LEFT){
            	tempScrollX = (getWidth() > 0) ? mWallpaperIndex*getWidth()-(getWidth()-mScrollX%getWidth()) : mScrollX;
            } else if (mTouchDirection==TOUCH_STATE_SCROLLING_RIGHT) {
            	tempScrollX = (getWidth() > 0) ? mWallpaperIndex*getWidth()+mScrollX%getWidth() : mScrollX; 
            } else {
            	tempScrollX = mScrollX;
            }
            
            if (mNextScreen!=INVALID_SCREEN && mTouchState==TOUCH_STATE_REST && mScrollX%getWidth()==0){
            	CellLayout current = (CellLayout) getChildAt(mCurrentScreen);
            	mWallpaperIndex = current.getPageIndex();
            	tempScrollX = mWallpaperIndex*getWidth();
            }
            Log.i(TAG, "updateWallpaperOffset, mWallpaperIndex="+mWallpaperIndex+",mScrollX="+mScrollX+",tempScrollX="+tempScrollX+",mNextScreen="+mNextScreen+",mTouchState="+mTouchState);
            if (tempScrollX > (float) scrollRange) {
            	//tempScrollX -= scrollRange;
            	int alpha = 255-(int)(((tempScrollX-scrollRange)*1.0/getWidth()*1.0)*255);
            	Log.i(TAG, "updateWallpaperOffset, alpha="+alpha+",tempScrollX="+tempScrollX);
            	mWallpaperManager.getDrawable().setAlpha(alpha);
            	//mWallpaperManager.getFastDrawable().setAlpha(alpha);
            }
            mWallpaperManager.setWallpaperOffsets(getWindowToken(),
                    Math.max(0.0f, Math.min(tempScrollX / (float) scrollRange, 1.0f)), 0);
        }
    }
...
//           	mCueNumber.drawCueNumber(canvas, mPaint, this.getWidth(), mCueBitmap);
//           }
    }
...
//                 if (mDragInfo != null && isViewSpan1x1(mDragInfo.cell) && mViewWidth > 0) {
//                   	 //CellLayout current = (CellLayout) getChildAt(mCurrentScreen);
//                   	 mStartDrag = true;             
//                   	 startDrag(mDragInfo, (int) ev.getX(), (int) ev.getY());
//                   	 //startDrag((int) ev.getX(), (int) ev.getY());
//                   	 //mDragController.startDrag(mDragInfo.cell, this, mDragInfo.cell.getTag(), DragController.DRAG_ACTION_MOVE);
//                 }
                     ...
                     if (!currentScreen.lastDownOnOccupiedCell()) {
                         ...
                         if (pointerIndex >= 0) {
	                         try{
	                        	 ...
	                        	 Log.e(TAG, "onInterceptTouchEvent sendWallpaperCommand error!  " +
...
//                 }
	...
	/*private void endDrag(CellLayout current, int x, int y) {
		if (mDragInfo == null) 
			return;
		
		View mDragView = mDragInfo.cell;
		//CellLayout current = (CellLayout) getChildAt(mCurrentScreen);
		
		if (mDragView != null) {
			if (isDropOnDeleteZone) {
				//onDrop(this, x, y, 0, 0, mDragInfo.cell,  mDragInfo);
				//mLauncher.mDeleteZone.onDrop(this, x, y, 0, 0, mDragInfo.cell,   mDragView.getTag());
				return;
			}
...
//			int point[] = new int[2];
//			current.cellToPoint(newCell[0], newCell[1], point);	
//			mDragView.layout(point[0], point[1], point[0]+mDragView.getWidth(), point[1]+mDragView.getHeight());
			
			if (mDragInitPos != mToPos) {
				//current.removeViewAt(mDragInitPos);
				//current.addView(mDragView, mToPos);			
				//this.exchangeScreen(mDragInitPos, mToPos);
				this.exchangeTheCell(mCurrentScreen, mDragInitPos, mToPos, true);
			}
			
			//removeViewAt(current.getChildCount() - 1);
			//current.addView(mDragView, mToPos);
			mDragView = null;
			mDragInfo = null;
   ...
   /* void enableChildrenCache(int fromScreen, int toScreen) {
    	...
    	if (fromScreen==count-1 && toScreen==0) {
    	...
    	if (fromScreen==0 && toScreen==count-1) {
    		final CellLayout layout2 = (CellLayout) getChildAt(0);
            layout2.setChildrenDrawnWithCacheEnabled(true);
            layout2.setChildrenDrawingCacheEnabled(true);
         ...
         switch (action & MotionEvent.ACTION_MASK) {
                 ...
                 if (mTouchState == TOUCH_STATE_SCROLLING) {
                     ...
                     if (deltaX < 0) {
                         ...
                         if (mTouchX > 0) {
                        	 ...
                        	 mTouchX -= getChildAt(getChildCount() - 1).getRight() + deltaX;
                        	 ...
                        	 mTouchX += deltaX;
                             //mTouchX += getChildAt(getChildCount() - 1).getRight() + deltaX;
                             //mScrollX = (int)mTouchX;
                             mSmoothingTime = System.nanoTime() / NANOTIME_DIV;
                             invalidate();
                         } else {
                        	 changChildWhenScrollLeft();
                        	 //jointFirst2Last();
                        	 mTouchX += getWidth()+deltaX;
                        	 mScrollX = (int)mTouchX;
                             mSmoothingTime = System.nanoTime() / NANOTIME_DIV;
                             invalidate();
                         }*/
                         ...
                         final float availableToScroll =
                        	 getChildAt(getChildCount() - 1).getRight() - mTouchX - getWidth();
                         if (availableToScroll > 0) {
                        	 ...
                        	 mTouchX += deltaX;
                        	 //if (mTouchX >= getChildAt(getChildCount()-1).getRight())
                        		// mTouchX -= getChildAt(getChildCount()-1).getRight();
                             mSmoothingTime = System.nanoTime() / NANOTIME_DIV;
                             invalidate();
                         } else {
             ...
             case MotionEvent.ACTION_UP:
                 if (mTouchState == TOUCH_STATE_SCROLLING) {
                     ...
                     //if (whichScreen>getChildCount()-1) whichScreen = getChildCount()-1;
...
//        }
        ...
        if ((mCurrentScreen == 0) && (mNextScreen == (getChildCount() - 1))) {
        	screenDelta = 1;
        	//mScrollX = (getChildCount()-1)*getWidth();
        	//newX = whichScreen * getWidth();
        } else if ((mCurrentScreen == (getChildCount() - 1)) && (mNextScreen == 0)) {
        	screenDelta = 1;
        	//mScrollX = 0;// -1 * getWidth();
        	newX = getChildCount()*getWidth();
        } else {
        	screenDelta = Math.max(1, Math.abs(whichScreen - mCurrentScreen));
        	//newX = whichScreen * getWidth();
        }*/
...
//	}
	...
	private int getXPos(int index, int thumbWidth) {
		...
		int pos=thumbWidth * (index % mColCount) + mWidthStartPadding + offsetX;// - 3;
		Log.i(TAG,"moveitem,xpos="+pos);
		return pos;
	}
	...
	private int getYPos(int index, int thumbHeight) {
		...
		int pos=thumbHeight * (index / mRowCount) + mHeightStatusBar + offsetY + mHeightStartPadding ;// + 12;//39; //magic number : 200 - 39 = 161;
		Log.i(TAG,"moveitem,ypos="+pos+",mHeightStatusBar="+mHeightStatusBar);
		return pos;
		
	}
	
	public void startItemAnimate(int x1, int y1, int x2, int y2, int w, int h, View child) {		
	...
	public void startItemAnimate(int x1, int y1, int x2, int y2, View child) {
		if (child != null) {
			//startItemAnimate(x1, y1, x2, y2, child.getWidth(), child.getHeight(), child);
...
//				
//				View child = current.getChildAt(getaMap(init));//(current.numberToIndex(init));
...
//						break;
//					} else {
//						CellLayout.LayoutParams lp = (CellLayout.LayoutParams) temp.getLayoutParams();
...
//		View temp = null;
//		
//		fromPos = current.findNearestVacantCellBetween(fromPos, toPos); //pass empty cell
//		
//		if (fromPos < toPos) {
...
//		mFromPos = fromPos;
//		mToPos = toPos;
//		Log.i(TAG,"914,from-to:"+mFromPos+"-"+mToPos);
//		
...
//				
//				View child = current.getChildAt(current.numberToIndexLastTime(init));//(current.numberToIndex(init));
...
//		}
		...
		Log.i(TAG,"914,one move sesion finished");
	...
	void moveItem(int init, int offset, View child, CellLayout current) {		
		...
		Log.i(TAG, "moveItem,(x1,y1)=("+x1+","+y1+"),(x2,y2)=("+x2+","+y2+")");
		
		startItemAnimate(x1, y1, x2, y2, child);		
		
		current.numberToCell(init+offset, newCell);
		current.changeCellXY(child, newCell[0], newCell[1], x2, y2-mHeightStatusBar);
		//current.changeCellXY(child, newCell[0], newCell[1]);
...
//		
//		int x2 = mWidthStartPadding + cellXY[0]*mCellWidth + offsetX;//getXPos(mToPos - 1, mCellWidth);
//		int y2 = mHeightStartPadding + cellXY[1]*mCellHeight + offsetY;//getYPos(mToPos - 1, mCellHeight);
//		
//		mItemAnimate.stop();
//		mItemAnimate.setAnimateTarget(x1, x2, y1, y2, child);
//		mItemAnimate.setDuration(600);
//		mItemAnimate.setSquare(child.getMeasuredWidth(), child.getMeasuredHeight());
//		mItemAnimate.start();
//
//	}
...
//	}
	...
	void startDrag(CellLayout current, CellLayout.CellInfo cellInfo, int fromPos, int toPos) {	
		if (current==null || cellInfo==null || cellInfo.cell==null) return;
		if (toPos<0 || toPos>current.getMaxCount()-1) return;
...
//		}
		...
		if (mItemAnimate.animateEnd) {
			//if (view!=cellInfo.cell) {
				//startAnimate(current, current.cellToNumber(cellInfo.cellX, cellInfo.cellY), current.cellToNumber(newCell));	
				startAnimate(current, fromPos, toPos);
				//mDragController.startDrag(mDragInfo.cell, this, mDragInfo.cell.getTag(), DragController.DRAG_ACTION_MOVE);
			//}
		} else {
			Log.i(TAG,"914, animateEnd=false");
		}
...
//        	temp = Math.abs(current.findFirstVacantCell()-temp) < Math.abs(current.findLastVacantCell()-temp) ? current.findFirstVacantCell() : current.findLastVacantCell();
...
//		mToPos = current.cellToNumber(newCell[0], newCell[1]);//findViewIndex(newCell[0], newCell[1]);//findViewIndex(view, 0);
...
////		}
////		
////		Log.i(TAG, "yfzhao-"+view.getClass().toString());
////		
////		if (view instanceof DockButton) {
////			//mLauncher.getWorkspace().exchangeAllCells(mLauncher.getWorkspace().getCurrentScreen()); //yfzhao
////		} else {
////			//mLauncher.getWorkspace().exchangeAllCells(mLauncher.getWorkspace().getCurrentScreen()); //yfzhao
////		}
//		
...
//		
//		for (int i = 0; i < current.getChildCount(); i++) {
//			View child = (viewId == 0) ? current.getChildAt(i) : current.getChildAt(i).findViewById(viewId);
//			if (child == v) {
//				index = i;
//				break;
//			}
...
//		
//		//index = cellInfo.cellY*4+cellInfo.cellX;  //getTheDropView(cellInfo.cellX, cellInfo.cellY);
//		
//		//return index;
...
//	}
    ...
    public void setLongClickValues(CellLayout.CellInfo cellInfo) {
    	...
    	CellLayout current = (CellLayout) getChildAt(mCurrentScreen);
    	//current.checkCellLayout();
		mDragInitPos = current.cellToNumber(cellInfo.cellX, cellInfo.cellY);//findViewIndex(cellInfo);//findViewIndex(v, 0);
		...
		//mViewHeight = v.getHeight()>0?v.getHeight():mLauncher.mItemHeight;//v.getMeasuredHeight();
		
		
		CellLayout.LayoutParams lp = (CellLayout.LayoutParams) v.getLayoutParams();
		Log.i(TAG,"moveitem,lp="+lp.toString());
		if(lp.width<=0)
			lp.width=mViewWidth;
		if(lp.height<=0)
			lp.height=mViewHeight;
		
		Log.i(TAG,"moveitem,getMeasuredWidth="+getMeasuredWidth()+",getMeasuredHeight="+getMeasuredHeight());
		Log.i(TAG,"moveitem,getLeft="+getLeft()+",getRight="+getRight()+",getTop="+getTop()+",getBottom="+getBottom());
		Log.i(TAG,"moveitem,getWidth()+"+getWidth()+"getHeight()="+getHeight());
		...
		Log.i(TAG,"moveitem,Width()+"+Launcher.mScreenWidth+"Height()="+Launcher.mScreenHeight);
		
		mCellWidth = (width - mWidthStartPadding - mWidthEndPadding) / mColCount;
		mCellHeight = (height - mHeightStartPadding - mHeightEndPadding - mHeightStatusBar) / mRowCount;

		//v.getParent().bringChildToFront(v);
		//v.layout(getPosX(mPos[0]), getPosY(mPos[1]), getPosX(mPos[0]) + mCellWidth, getPosY(mPos[1]) + mCellHeight);
    }
    ...
    void startDrag(CellLayout.CellInfo cellInfo) {
        ...
        if(getResources().getBoolean(R.bool.config_lock_apps)){
	        ...
	        if(tag instanceof ShortcutInfo){
	        	...
	        	if (intent.getComponent() != null ) {
	                ...
	                if(pkgName.equals("com.mediatek.wo3g") && className.equals("com.mediatek.wo3g.Wo3G")){
	                    return;
	                }
                }
                //]]at 2012-5-23
        ...
        CellLayout.LayoutParams lp = (CellLayout.LayoutParams)child.getLayoutParams();
        Log.i(TAG,"moveitem,startdrag lp="+lp.toString());
        
        //if (isViewSpan1x1(mDragInfo.cell)) {
        	setLongClickValues(cellInfo);
        	//mLauncher.mDeleteZone.onDragStart(this, child.getTag(), DragController.DRAG_ACTION_MOVE);
        //} //else {
...
//    public void moveTheCellView(DragSource source, Object dragInfo, CellLayout cellLayout, int[] oldCell, int[] newCell, int index) {
//    	//
//    	int countX = cellLayout.getCountX();
//    	//int countY = cellLayout.getCountY();
//    	
//    	int oldPlace = cellLayout.cellToNumber(oldCell[0], oldCell[1]); //oldCell[1]*countX+oldCell[0];
//    	int newPlace = cellLayout.cellToNumber(newCell[0], newCell[1]); //newCell[1]*countX+newCell[0];
//    	
...
//			            	//for effect
//		    				//this.mDragController.startDrag(child, source, dragInfo, DragController.DRAG_ACTION_MOVE);
...
//    public void moveTheCellView(CellLayout cellLayout, int screen, View cell, int oldPlace, int newPlace) {
//    	
//    	int countX = cellLayout.getCountX();
//
...
//	    			
//	    			if (index >= 0) {		    			
//		    			if (cellLayout.getCellSpanX(index) == 1 && cellLayout.getCellSpanY(index) == 1)	 {
...
//    private void onDropDifferentScreen(View view, int srcScreen, int destScreen) {
...
//    }
	...
	boolean isViewMoveToSamePlace(View v, int fromScreen, int toScreen, int newCellX, int newCellY) {
		if (fromScreen != toScreen) {
			return false;
		} else {
			CellLayout.LayoutParams lp = (CellLayout.LayoutParams) v.getLayoutParams();
	...
	boolean isViewMoveToSamePlace(CellLayout.CellInfo cellInfo, int fromScreen, int toScreen, int newCellX, int newCellY) {
		if (fromScreen != toScreen) {
			return false;
		} else {
...
//	void onDropScreen(CellLayout.CellInfo cellInfo, int fromScreen, int toScreen, int newCellX, int newCellY){
//		int emptyNumber = -1;
//		int newNumber = -1;
//		View child = null;
//		
...
//				moveTheCellView(toLayout, toScreen, cellInfo.cell, emptyNumber, newNumber);
//
//			} else {
//				
//				CellLayout toLayout = (CellLayout) getChildAt(toScreen);
...
//			moveTheCellView(cellLayout, toScreen, cellInfo.cell, cellLayout.cellToNumber(cellInfo.cellX, cellInfo.cellY), cellLayout.cellToNumber(newCellX, newCellY));
//		}
//	}
	...
	public void onDrop(DragSource source, int x, int y, int xOffset, int yOffset,
    	...
    	Log.i(TAG,"drag sequence,workspace onDrop");
...
//            		cellLayout.pointToCellRounded(x - xOffset, y - yOffset, newCell);
//
//            		if (!(isViewMoveToSamePlace(mDragInfo, mDragInfo.screen, index, newCell[0], newCell[1]))) {
//            			//cellLayout.checkCellLayout();
//                		int dropIndex  = cellLayout.cellToIndex(newCell[0], newCell[1]);
//                		if (dropIndex >= 0) {	 
//                			if ((cellLayout.getCellSpanX(dropIndex) == 1) && (cellLayout.getCellSpanY(dropIndex) == 1)) {
//                				View dropView = cellLayout.getChildAt(dropIndex);
//                				if (!(dropView instanceof Folder)) {
//                					//onDropScreen(mDragInfo, mDragInfo.screen, index, newCell[0], newCell[1]);
//                	                onDropInternalEx(x, y , xOffset, yOffset, cell, cellLayout, index);                	                
//                	                exchangeAllCells(index);
//                					return;
//                				}
//                			}         			
//                		} 
//            		}
            		if(cellLayout.getPageIndex()!=itemInfo.screen){
	                ...
	                if (cellLayout.getPageIndex() != itemInfo.screen) {
	                	if(cellLayout.isFull() || !acceptDropInternal(source, x, y, xOffset, yOffset, dragView, dragInfo)){
	                		//cellLayout.removeView(mDragInfo.cell);
	                		//oriLayout.addView(mDragInfo.cell);
	                		//final CellLayout originalCellLayout = (CellLayout) getChildAt(itemInfo.screen);
	                    	//originalCellLayout.addView(cell);
	                		
	                		dragView.setmCallbackFlag(false);
	                		//mDragInfo.screen=mCurrentScreen;//oriLayout.getPageIndex();
            				...
            				cellLayout.addView(cell);
	   	                    //if(cellLayout.indexOfChild(cell)<0)
	   	                    //	cellLayout.addView(cell); 
	   	                    //mDragInfo.screen=cellLayout.getPageIndex();   
	                		onDropInternal(x, y , xOffset, yOffset, cell, cellLayout, mCurrentScreen);
	                	}
	                } else { // move in current page
	                	onDropInternal(x, y , xOffset, yOffset, cell, cellLayout, mCurrentScreen);
	                }
            	}
        ...
        Log.i(TAG,"dockbar, mIsEmpty,ondrop workspace from dockbar,current has "+ cellLayout.getChildCount()+" children");
    ...
    void onDropInternal(int x, int y, int xOffset, int yOffset,
            View cell, CellLayout cellLayout, int index) {
		...
		//cellLayout.changeCellXY(mDragInfo.cell, mTargetCell[0], mTargetCell[1]);
    ...
    void onDropInternalEx(int x, int y, int xOffset, int yOffset,
            View cell, CellLayout cellLayout, int index) {    	
	...
	public void onDragEnter(DragSource source, int x, int y, int xOffset, int yOffset,
    	...
    	Log.i(TAG,"drag sequence,workspace onDragEnter");
		...
		if (shortcutView==null) return;		
		
		int index = mScroller.isFinished() ? mCurrentScreen : mNextScreen;
		CellLayout current = (CellLayout) getChildAt(index);
		if (current==null) return;
		shortcutInfo.screen = index;
		current.pointToCellExact(x, y, newCell);//newCell?? pay attention to this
		if (current.cellToIndex(newCell)>=0) {
			int asPos = current.findNearestVacantCellIn(current.cellToNumber(newCell));
		...
		CellLayout.LayoutParams params = new CellLayout.LayoutParams(
				shortcutInfo.cellX, shortcutInfo.cellY, shortcutInfo.spanX, shortcutInfo.spanY);
		params.isDragging = true;	
		Log.i(TAG, "917, cell(x,y) = "+newCell[0]+","+newCell[1]);
		current.cellToPoint(shortcutInfo.cellX, shortcutInfo.cellY, newCell);
		Log.i(TAG, "917, params(x,y) = "+params.x+","+params.y+"newcell:"+newCell[0]+","+newCell[1]);
		params.x = newCell[0];
		params.y = newCell[1];
		...
		Log.i(TAG, "917, mDragInfo.screen"+ shortcutInfo.screen);
   
		Log.i(TAG,"dockbar, onDragEnter workspace,current has "+ current.getChildCount()+" children");
    }

    void updateDragInfo(CellLayout.CellInfo cellInfo, final int[] newCell, int screen){//,int x,int y){
    	...
    	if (cellInfo.cell==null) return;
    	
    	CellLayout.LayoutParams lp = (CellLayout.LayoutParams) cellInfo.cell.getLayoutParams();
    	if (lp==null) return;
    	...
    	Log.i(TAG, "914,updateDragInfo,sceen="+screen+"lp="+lp);
	...
	public void onDragOver(DragSource source, int x, int y, int xOffset, int yOffset,
    	...
    	int screen = -1;
    	Log.i(TAG,"drag sequence,workspace onDragOver");
...
//				} else if(index>mCurrentScreen){
//					//changChildWhenScrollRight();
//					screen = getCurrentScreen()-1;
//				}
//	    		CellLayout lastCellLayout = (CellLayout) getChildAt(screen);//(mOldScreen);
//	    		
//	    		lastCellLayout.removeView(mDragInfo.cell);
//	    		Log.i(TAG,"last remove child = "+lastCellLayout.getChildCount());
//	//    		if (current.isFull()) {
//	//    			mOldScreen = index;
...
//		    	if(!(source instanceof UserFolder) && current.indexOfChild(mDragInfo.cell)<0){
//		    		current.addView(mDragInfo.cell);
//		    		mDragInfo.screen=mCurrentScreen;//current.getPageIndex();
//		    	}
//		    	Log.i(TAG,"current add child = "+current.getChildCount());    	
//	    		//mOldScreen=mCurrentScreen;
//				
//	    	}     	
	    	
	    	assert(mCurrentScreen==2);
	    	current = (CellLayout) getChildAt(mCurrentScreen);
//	    	if (current==null){
...
//	    	}
	    	...
	    	if (current.isFull() && (source!=this || (current.indexOfChild(mDragInfo.cell)<0)) || !isViewSpan1x1(mDragInfo)) {
	    		//Toast.makeText(getContext(), getContext().getString(R.string.out_of_space), Toast.LENGTH_SHORT).show();
	    		mIsOverFinished=true;
	    		return;
	    	}
	        ...
	        Log.i(TAG,"914,(x,y)="+x+","+y+"cell("+overCell[0]+","+overCell[1]+")fromPos="+mFromPos+",toPos="+mToPos);
	        
	        int newIndex = current.cellToIndex(overCell);
	        
	    	if (newIndex>=0 && isViewSpan1x1(current.getChildAt(newIndex))){
	    		if (!current.getChildAt(newIndex).equals(mDragInfo.cell)) {	    	
		    		Log.i(TAG,"914,overCell is not empty,move the item in it "+mFromPos+"-"+mToPos);
		    		startDrag(current, mDragInfo, mFromPos, mToPos);
		        	Log.i(TAG,"914,async or sync?");
		        	mFromPos=mToPos;
		        	updateDragInfo(mDragInfo, overCell, current.getPageIndex());
	    		} else{
		        	mFromPos=mToPos;
		        	updateDragInfo(mDragInfo, overCell, current.getPageIndex());
	    			Log.i(TAG,"914, move in its own place, update for mistake");
	    		}
	    	} else {
	        	Log.i(TAG,"914,overCell is empty, update info and go ahead");
	        	mFromPos=mToPos;
	        	updateDragInfo(mDragInfo, overCell, current.getPageIndex());
	    	}
...
//        if (mOldScreen==INVALID_SCREEN) {
//        	int firstVacantPos = current.findFirstVacantCell();
//        	current.numberToCell(firstVacantPos, newCell);
//	        //int lastVacantPos = current.findLastVacantCell();
//	        mDragInfo.screen = index;
//    		mDragInfo.cellX = newCell[0];
//    		mDragInfo.cellY = newCell[1];
//    		mOldScreen = index;	    	
//        }
        
        //startDrag(mDragInfo, x, y);
//
//
//    	 
...
//	        	
//	    	} else {
//	    		Log.i(TAG, "Workspace.onDragOver - source is unknown");
//	    	}    	
...
//					(ViewGroup) getChildAt(info.screen), info);
//    		
//    		View view = uf.getChildAt(info.position);
//    		
//    		if (view.equals(shortcut)) {
//    			Log.i(TAG, "equal");
//    		}
//    		
//    		if (shortcut == null)
...
//    		
//    		CellLayout.CellInfo cellInfo = (CellLayout.CellInfo) shortcut.getTag();//new CellLayout.CellInfo();
//    		
//    		//if (cellInfo==null)
//    		//	return;
//    		
...
//    	}
	...
	public void onDragExit(DragSource source, int x, int y, int xOffset, int yOffset,
            DragView dragView, Object dragInfo) {
    	Log.i(TAG,"drag sequence,workspace onDragExit");
    	
        clearVacantCache();
        
        //yfzhao
        //1. from workspace, pass        
        //if (source instanceof Workspace) return;
    ...
    private void onDropExternal(int x, int y, Object dragInfo, CellLayout cellLayout,
        ...
        case Applications.APPS_TYPE_FOLDERAPP:
        	...
        	info = mLauncher.getLauncherModel().getShortcutInfo(getContext().getPackageManager(),
        ...
        LauncherModel.addOrMoveItemInDatabase(mLauncher, info,
                Favorites.CONTAINER_DESKTOP, cellLayout.getPageIndex()/*mCurrentScreen*/, lp.cellX, lp.cellY);
        ...
        if (((ItemInfo) dragInfo).itemType == Applications.APPS_TYPE_FOLDER) {
        	...
        	for (int i = 0; i < folderInfo.getSize(); i++) {
    			...
    			LauncherModel.addItemToDatabase(mLauncher, item, info.id, 0, 0, 0, false);
        	}
        	// If the folder has sub items, refresh folder's icon
        	if (((UserFolderInfo) info).getSize() > 0) {
        		((FolderIcon) ((UserFolderInfo) info).folderIcon).refreshFolderIcon();
        	}
...
//        if (source == this && layout.isFull() && (layout.getCellSpanX(dropIndex) == 1) && (layout.getCellSpanY(dropIndex) == 1))
//        	return true;
//        else*/
//        	return mVacantCache.findCellForSpan(mTempEstimate, spanX, spanY, false);
	...
	public void onDropCompleted(View target, boolean success) {
    	Log.i(TAG,"drag sequence,workspace onDropCompleted");
    	
        clearVacantCache();

        if (success) {
            if (target != this && mDragInfo != null) {
                final CellLayout cellLayout = (CellLayout) getChildAt(SettingUtils.mHomeScreenIndex);//(mDragInfo.screen);
                cellLayout.removeView(mDragInfo.cell);
            ...
            if (mDragInfo != null) {
            	...
            	if (oriLayout != null && mDragInfo.cell != null)
...
//		}
                    ...
                    for (int j = 0; j < childCount; j++) {
                        ...
                        if (tag instanceof ShortcutInfo) {
                            ...
                            if (removedFromFolder) {
                                ...
                                ((FolderIcon)info.folderIcon).refreshFolderIcon();
                            }
    ...
    void moveToDefaultScreen(boolean animate) {
    	int defaultChild=getChildIndexByPos(mDefaultScreen);
	...
	protected boolean getChildStaticTransformation(View child, Transformation t) {
		...
		EffectBase effect =
			EffectsFactory.getEffectByType(SettingUtils.mTransitionEffect);
		if (effect == null) {
			return false;
		}

		float ratio = getCurrentScrollRatio(child);
		return effect.getWorkspaceChildStaticTransformation(this, child,
				t, mCamera, ratio, mCurrentScreen,
	...
	public float getCurrentScrollRatio(View view) {
		...
		float ratio = (workspaceOffset - viewOffset) / (workspaceWidth + viewWidth) * 2.0f;
	...
	private void setScreenTransitionType(int type) {
		if (mTransitionType != type) {
			...
			for (int i = 0; i < getChildCount(); i++) {
				((CellLayout) getChildAt(i)).setStaticTransformationsEnabled(bEnableTrans);
			}
		}
	}
    ...
    final void switchScreenMode(boolean bIsFullScreen, int paddingTop) {
    	for (int i = 0; i < getChildCount(); i++) {
    		((CellLayout) getChildAt(i)).switchScreenMode(bIsFullScreen, paddingTop);
    	}
    	requestLayout();
    }
    ...
    private void processItemsInScreen(int screenIndex) {
    	...
    	for (int i = 0; i < cell.getChildCount(); i++) {
            ...
            if (item.container == Favorites.CONTAINER_DESKTOP) {
                if (item instanceof LauncherAppWidgetInfo) {
                    ...
                    if (((CustomAppWidgetInfo) item).itemType ==
                        ...
                        // should remove administration when no more LOCK_SCREEN widget displayed in launcher
                        if (c.getCount() <= 1) {
    	...
    	// screen = screen - 1
		final ContentResolver cr = mLauncher.getContentResolver();
		cr.update(LauncherProvider.CONTENT_DELETE_SCREEN_URI,
				null, null, new String[]{String.valueOf(cell.getPageIndex()/*screenIndex*/)});
    }
...
//    }
	...
	public int getChildIndexByPos(int pageIndex){
		int result = -1;
	...
	public void exchangeScreen(int fromPos, int toPos) {
		int fromChildIndex = getChildIndexByPos(fromPos);
		int toChildIndex = getChildIndexByPos(toPos);
		
		if(fromPos>toPos){
			for(int i=toPos;i<fromPos;i++){
				CellLayout layout=(CellLayout)getChildAt(this.getChildIndexByPos(i));
		...
		final Uri uri = (toPos > fromPos) ?
				LauncherProvider.CONTENT_MOVE_FORWARD_SCREEN_URI :
				LauncherProvider.CONTENT_MOVE_BACKWARD_SCREEN_URI;
		cr.update(uri, null, null, new String[] {String.valueOf(fromPos), String.valueOf(toPos)});
		...
		where.append(")");
		cr.update(Favorites.CONTENT_URI_NO_NOTIFICATION,
				values, where.toString(), null);
		// 3. Process current screen and home screen
//		int currentpageIndex = ((CellLayout)this.getChildAt(mCurrentScreen)).getPageIndex();
...
//		}
    ...
    public void exchangeTheCell(int index, int fromPos, int toPos, boolean only1x1) {
    	CellLayout current = (CellLayout) getChildAt(index);
    	View child = null;
...
//	}
    ...
    public void printChildCount(){  
    	Log.i(TAG,"workspace default screen is "+this.mDefaultScreen+", current screen is "+ this.mCurrentScreen);
    	for(int i=0;i<this.getChildCount();i++){
    		CellLayout layout=(CellLayout)this.getChildAt(i);
    		Log.i(TAG,"workspace has "+this.getChildCount()+","+i+" child has "+layout.getChildCount()+" cells and pageIndex is "+layout.getPageIndex());
    	}
...
//				"mCurrentScreen="+mCurrentScreen+", mDefaultScreen="+mDefaultScreen);
	...
	private void applyThemeOnCellLayout(CellLayout cellLayout) {
		for (int i = 0; i < cellLayout.getChildCount(); i++) {
			...
			if (tag instanceof ShortcutInfo
					/*&& ((ShortcutInfo) tag).itemType == Favorites.ITEM_TYPE_APPLICATION*/) {
...
//		        	Drawable d1 = res.getDrawable(iconId);
//		        	Bitmap icon = Utilities.drawable2bmp(d1);
//	        		Bitmap bmp = Utilities.createCompoundBitmapEx(Integer.toString(winfo.title), icon);
...
//			                null, null);
	...
	/**
	 * @param mDragInfo the mDragInfo to set
	 */
	...
	/**
	 * @param mTouchDirection the mTouchDirection to set
	 */
