...
import com.vollo.launcher.theme.ThemeUtils;
...
public class LauncherProvider extends ContentProvider {
    ...
    // Used to identify shortcuts in favorite is intent with action or intent with component name
    private static final String FAVORITE_ACTION = "action";

    private static final int DATABASE_VERSION = 10;

    static final String AUTHORITY = "com.vollo.launcher.settings";
    ...
    static final Uri CONTENT_DELETE_SCREEN_URI =
        Uri.parse("content://" + AUTHORITY + "/favorites/deletescreen");

    static final Uri CONTENT_MOVE_FORWARD_SCREEN_URI =
        Uri.parse("content://" + AUTHORITY + "/favorites/moveforwardscreen");

    static final Uri CONTENT_MOVE_BACKWARD_SCREEN_URI =
        Uri.parse("content://" + AUTHORITY + "/favorites/movebackwardscreen");
    ...
    static {
    	...
    	mUriMatcher.addURI(AUTHORITY, "favorites/deletescreen", FAVORITE_DELETE_SCREEN);
    	mUriMatcher.addURI(AUTHORITY, "favorites/moveforwardscreen", FAVORITE_MOVE_FORWARD_SCREEN);
    	mUriMatcher.addURI(AUTHORITY, "favorites/movebackwardscreen", FAVORITE_MOVE_BACKWARD_SCREEN);
    	mUriMatcher.addURI(AUTHORITY, "favorites/adjustOrderId", FAVORITE_ADJUST_ORDERID);
    	...
    	mUriMatcher.addURI(AUTHORITY, "applications/insertfolder", INSERT_FOLDER);
    	...
    	mUriMatcher.addURI(AUTHORITY, "applications/adjustOrderId", ADJUST_ORDERID);
    ...
    public int update(Uri uri, ContentValues values, String selection, String[] selectionArgs) {
    	...
    	case FAVORITE_DELETE_SCREEN:
    		...
    		String sqlDelete = String.format("update %s set %s=%s-1 where %s>%s", paramsDelete);
    	...
    	case FAVORITE_MOVE_FORWARD_SCREEN:
    		...
    		String sqlForward = String.format("update %s set %s=%s-1 where %s>%s and %s<=%s", paramsForward);
    	...
    	case FAVORITE_MOVE_BACKWARD_SCREEN:
    		...
    		String sqlBackford = String.format("update %s set %s=%s+1 where %s>=%s and %s<%s", paramsBackward);
    	...
    	case FAVORITE_ADJUST_ORDERID:
			...
			String sqlAdjustFav = String.format("update %s set %s=%s-%d where %s=%s and %s>%s", paramsAdjustFav);

			db.execSQL(sqlAdjustFav);
			break;
		case APPLICATIONS:
			count = db.update(TABLE_APPLICATIONS, values, selection, selectionArgs);
			getContext().getContentResolver().notifyChange(uri, null);
        	break;
    	case APPLICATION_ID:
            SqlArguments argsApps = new SqlArguments(uri, selection, selectionArgs);

            count = db.update(argsApps.table, values, argsApps.where, argsApps.args);
            break;
		case ADD_FOLDER:
			...
			String sqlAdd = String.format("update %s set %s=%s+%d where %s>=%s", paramsAdd);
		...
		case INSERT_FOLDER:
			...
			String sqlInsert = String.format("update %s set %s=%s-%d where %s>%s", paramsInsert);
		...
		case MOVE_FRONT:
			...
			String sqlFront = String.format("update %s set %s=%s+%d where %s>=%s and %s<%s", paramsFront);
		...
		case MOVE_BEHIND:
			...
			String sqlBehind = String.format("update %s set %s=%s-%d where %s>%s and %s<=%s", paramsBehind);
		...
		case ADJUST_ORDERID:
			...
			String sqlAdjust = String.format("update %s set %s=%s-%d where %s=%s and %s>%s", paramsAdjust);
		...
		case ADD_STARTNUM:
			...
			String sqlAddNum = String.format("update %s set %s=%s+%d where %s='%s'", paramsAddNum);
    ...
    private static class DatabaseHelper extends SQLiteOpenHelper {
        ...
        private int copyFromCursor(SQLiteDatabase db, Cursor c) {
            ...
            final int iconTypeIndex = c.getColumnIndexOrThrow(BaseLauncherColumns.ICON_TYPE);
            final int iconIndex = c.getColumnIndexOrThrow(BaseLauncherColumns.ICON);
            final int iconPackageIndex = c.getColumnIndexOrThrow(BaseLauncherColumns.ICON_PACKAGE);
            final int iconResourceIndex = c.getColumnIndexOrThrow(BaseLauncherColumns.ICON_RESOURCE);
	     ...
	     */
	     static boolean loadTopPackage(Context context) {
	        ...
	        try {
	            ...
	            XmlResourceParser parser = Configurator.getConfigPackageXml(context, "default_toppackage"); 
	            if(parser == null){
	            	ctx = context;
	            	parser = context.getResources().getXml(R.xml.default_toppackage);
	            }
	            ...
	            int type;
                int [] attrsTopPackage = Configurator.getPackageAttrs(ctx, Configurator.sTopPackage);
                HashMap<String, Integer> attrMap = Configurator.getPackageStyleable(ctx, 
                		attrsTopPackage, Configurator.sTopPackage);
	            ...
	            while (((type = parser.next()) != XmlPullParser.END_TAG || parser.getDepth() > depth) 
	                ...
	                TypedArray a = ctx.obtainStyledAttributes(set, attrsTopPackage);                    
	                
	                mTopPackages.add(new TopPackage(a.getString(attrMap.get(Configurator.TOPPACKAGENAME)),
	                		a.getString(attrMap.get(Configurator.TOPCLASSNAME)),
	     ...
	     List<ResolveInfo>  reorderApplist(List<ResolveInfo> added) {
	         ...
	         ArrayList<ResolveInfo> dataReorder =
	             new ArrayList<ResolveInfo>(DEFAULT_APPLICATIONS_NUMBER); 
	         ...
	         for (TopPackage tp : mTopPackages) { 
	         	...
	         	for (ResolveInfo ai : added) {
	         		...
	         		if (ai.activityInfo.applicationInfo.packageName.equals(tp.mPackageName) 
		...
		private void loadAllApps(SQLiteDatabase db) {
	        ...
	        for (int j = 0; j < data.size(); j++) {
	            ...
	            if (appInfo.packageName.startsWith(ThemeUtils.THEME_PACKAGE_TOKEN) || 
	            		appInfo.packageName.equals(selfPkgName)) {
	            	continue;
	            }

	            String intentInfo = appInfo.packageName + "|" + info.activityInfo.name;
		        ...
		        // ]]at 2012-6-14
	        	values.put(Applications.PACKAGENAME, info.activityInfo.applicationInfo.packageName);
        		...
        		if (!(    actInfoName.equals("com.android.contacts.DialtactsActivity")
        				||actInfoName.equals("com.android.contacts.DialtactsContactsEntryActivity")
        				||actInfoName.equals("com.android.mms.ui.ConversationList")
        				||actInfoName.equals("com.android.browser.BrowserActivity")
        				
        				||actInfoName.equals("com.android.contacts.DialtactsCallLogEntryActivity")
        				||actInfoName.equals("com.android.calculator2.Calculator")
        				||actInfoName.equals("com.android.soundrecorder.SoundRecorder")
        				||actInfoName.equals("com.mediatek.StkSelection.StkSelection")
        				
        				||actInfoName.equals("com.mediatek.FMRadio.FMRadioActivity")
        				||actInfoName.equals("com.android.email.activity.Welcome")
        				||actInfoName.equals("com.android.calendar.LaunchActivity")
		        	...
		        	if ((info.activityInfo.name.indexOf(appInfo.packageName)) >= 0){
				        	intentInfo = "#Intent;action=android.intent.action.MAIN;category=android.intent.category.LAUNCHER;launchFlags=0x10200000;component=" 
				        				+ appInfo.packageName + "/" + info.activityInfo.name.substring(info.activityInfo.name.indexOf(appInfo.packageName)+appInfo.packageName.length()) + ";end";
		        		
		        	} else {
			        	intentInfo = "#Intent;action=android.intent.action.MAIN;category=android.intent.category.LAUNCHER;launchFlags=0x10200000;component=" 
		        				+ appInfo.packageName + "/" + info.activityInfo.name + ";end";
		        	}
	        	
		            ContentValues values2 = new ContentValues();
		            
		            values2.put(BaseLauncherColumns.TITLE, info.loadLabel(packageManager).toString());
		             
		            values2.put(BaseLauncherColumns.INTENT, intentInfo);
		            values2.put(Favorites.CONTAINER, Favorites.CONTAINER_DESKTOP);
		    		...
		    		//}
		        	//values2.put(Favorites.PACKAGENAME, info.activityInfo.applicationInfo.packageName);
		        	//values.put(Applications.INSTALL, info.activityInfo.la)
		        	//values2.put(Favorites.STARTNUM, 0);
					values2.put(BaseLauncherColumns.ICON_TYPE, BaseLauncherColumns.ICON_TYPE_RESOURCE);
					//values2.put(Favorites.ICON_PACKAGE, info.activityInfo.applicationInfo.packageName);
					//values2.put(Favorites.ICON_RESOURCE, info.getIconResource());
					
					long res2 = db.insert(TABLE_FAVORITES, null, values2); //yfzhao 
	        	
		        	if (res2 == -1) {
		        		Log.d(TAG, "loadAllApps TABLE_FAVORITES insert db error");
		        	} else {        		
		        			position2++;
		        	}	
	        	}
        ...
        private boolean updateContactsShortcuts(SQLiteDatabase db) {
            Cursor c = null;
            final String selectWhere = buildOrWhereString(BaseLauncherColumns.ITEM_TYPE,
                    new int[] { BaseLauncherColumns.ITEM_TYPE_SHORTCUT });

            db.beginTransaction();
            try {
                // Select and iterate through each matching widget
                c = db.query(TABLE_FAVORITES, new String[] { BaseColumns._ID, BaseLauncherColumns.INTENT },
                ...
                final int intentIndex = c.getColumnIndex(BaseLauncherColumns.INTENT);
        ...
        private void convertWidgets(SQLiteDatabase db) {
            ...
            final String selectWhere = buildOrWhereString(BaseLauncherColumns.ITEM_TYPE, bindSources);
            ...
            try {
                // Select and iterate through each matching widget
                c = db.query(TABLE_FAVORITES, new String[] { BaseColumns._ID, BaseLauncherColumns.ITEM_TYPE },
                        selectWhere, null, null, null, null);
                ...
                while (c != null && c.moveToNext()) {
                    ...
                    try {
                        ...
                        values.clear();
                        values.put(BaseLauncherColumns.ITEM_TYPE, Favorites.ITEM_TYPE_APPWIDGET);
        ...
        private int loadFavorites(SQLiteDatabase db) {
            ...
            try {
	            ...
	            XmlResourceParser parser = Configurator.getConfigPackageXml(mContext, "default_workspace"); 
	            if(parser == null){
	            	context = mContext;
                ...
                int type;
                int [] attrs = Configurator.getPackageAttrs(context, Configurator.sFavorite);
                HashMap<String, Integer> attrMap = Configurator.getPackageStyleable(context, 
                		attrs, Configurator.sFavorite);
                ...
                while (((type = parser.next()) != XmlPullParser.END_TAG ||
                    ...
                    values.put(Favorites.SCREEN, a.getString(attrMap.get(Configurator.SCREEN)));
                    values.put(Favorites.CONTAINER,
                    		a.getInt(attrMap.get(Configurator.CONTAINER), Favorites.CONTAINER_DESKTOP));
                    values.put(Favorites.CELLX, a.getString(attrMap.get(Configurator.CELLX)));
                    values.put(Favorites.CELLY, a.getString(attrMap.get(Configurator.CELLY)));

                    if (TAG_FAVORITE.equals(name)) {
                        added = addAppShortcut(db, values, a, attrMap, packageManager, intent);
                    } else if (TAG_SEARCH.equals(name)) {
                        added = addSearchWidget(db, values);
                    } else if (TAG_CLOCK.equals(name)) {
                        added = addClockWidget(db, values);
                    } else if (TAG_APPWIDGET.equals(name)) {
                        added = addAppWidget(db, values, a, attrMap, packageManager);
        ...
        private boolean addAppShortcut(SQLiteDatabase db, ContentValues values, TypedArray a,
        		HashMap<String, Integer> attrMap, PackageManager packageManager, Intent intent) {
            ActivityInfo info;
            ...
            if (type != null && type.equals(FAVORITE_ACTION)) {
                ...
                try {
                    ...
                    List<ResolveInfo> list =
                    	packageManager.queryIntentActivities(actionIntent, 0);
                    ...
                    if (appInfo != null) {
                    	ComponentName cn =
                    		new ComponentName(appInfo.activityInfo.packageName,
                        ...
                        values.put(BaseLauncherColumns.INTENT, intent.toUri(0));
                        values.put(BaseLauncherColumns.TITLE, appInfo.activityInfo.loadLabel(packageManager).toString());
                    ...
                    Log.w(TAG, "Unable to add favorite: " + action + "/" + uri, e);
                    return false;
                }
            } else {
                String packageName = a.getString(attrMap.get(Configurator.PACKAGENAME));
        ...
        private boolean addLockScreenAppWidget(SQLiteDatabase db, ContentValues values) {
        	values.put(BaseLauncherColumns.ITEM_TYPE, Favorites.ITEM_TYPE_WIDGET_LOCK_SCREEN);
        	db.insert(TABLE_FAVORITES, null, values);
        	return true;
        }
        
        private boolean addCleanMemAppWidget(SQLiteDatabase db, ContentValues values) {
        	values.put(BaseLauncherColumns.ITEM_TYPE, Favorites.ITEM_TYPE_WIDGET_CLEAN_MEMORY);
        	db.insert(TABLE_FAVORITES, null, values);
        	return true;
        }

        private boolean addAppWidget(SQLiteDatabase db, ContentValues values, TypedArray a,
        		HashMap<String, Integer> attrMap, PackageManager packageManager) {
            String packageName = a.getString(attrMap.get(Configurator.PACKAGENAME));
            String className = a.getString(attrMap.get(Configurator.CLASSNAME));
        ...
        private boolean addUriShortcut(SQLiteDatabase db, ContentValues values,
            ...
            final int iconResId = a.getResourceId(attrMap.get(Configurator.ICON), 0);
            final int titleResId = a.getResourceId(attrMap.get(Configurator.TITLE), 0);
            ...
            values.put(BaseLauncherColumns.ICON_PACKAGE, mContext.getPackageName());
