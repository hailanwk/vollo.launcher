...
//    private Animation mHandleOutAnimation;
	...
	public void onDrop(DragSource source, int x, int y, int xOffset, int yOffset,
		...
		if ((item.itemType == 0) || (item.itemType == 1)) {
			...
			//Intent it = data.getParcelableExtra(Intent.EXTRA_SHORTCUT_INTENT);
    ...
    private void deleteFromAllApps(ItemInfo itemInfo) {
		...
		case Applications.APPS_TYPE_FOLDERAPP:
			String pkgName =
				((ApplicationInfoEx) itemInfo).intent.getComponent().getPackageName();
			Uri uri = Uri.parse("package:" + pkgName);
			Intent intent = new Intent(Intent.ACTION_DELETE, uri);
			intent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
	...
	public void onDragStart(DragSource source, Object info, int dragAction) {
        ...
        if (item != null) {
            ...
            mRegion.set(location[0], location[1] - paddingTop,
